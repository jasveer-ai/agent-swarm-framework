...
def _failure_detail(value: Any) -> str:
    """Extract the most actionable nested provider error message."""
    ...
    if isinstance(value, str):
        try:
            ...
            return value
        return _failure_detail(nested)
    if isinstance(value, Mapping):
        for key in ("message", "error", "detail"):
            if key in value and value[key] not in (None, ""):
                return _failure_detail(value[key])
        return json.dumps(dict(value), sort_keys=True)
    if isinstance(value, list):
        details = [_failure_detail(item) for item in value]
...
def parse_jsonl_output(raw: str) -> Tuple[str, TokenUsage, int]:
    ...
    for line in raw.splitlines():
        ...
        if event.get("type") in {"error", "turn.failed"}:
            ...
            failures.append(redact_diagnostic(_failure_detail(detail), limit=500))
