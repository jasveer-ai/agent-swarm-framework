...
from agent_swarm.core.concurrency import WorkspaceGate
from agent_swarm.core.config import AgentProfile, SwarmConfig
from agent_swarm.core.protocol import Message
from agent_swarm.core.routing import NoEligibleWorker, WorkerSelector
from agent_swarm.core.run import (
    ReviewDecision,
    RunRecord,
    SwarmRunResult,
    TaskOutcome,
...
from agent_swarm.core.runtime import (
    AgentRuntime,
    BudgetExceeded,
    GenerationContractError,
)
from agent_swarm.providers.base import ModelProvider, ProviderError, redact_diagnostic
from agent_swarm.providers.cli import build_providers
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            budget_overage = runtime.ledger.overage_reason()
            if budget_overage:
            ...
            aggregation_message = Message(
                ...
                payload={
                    ...
                    "budget_overage_reason": budget_overage,
                },
                run_id=record.run_id,
        ...
        events = tuple(bus.get_history(limit=None))
        if self.config.output.include_bus_history:
            record.bus_history = list(events)
        ...
        return SwarmRunResult(final_output=final_output, record=record, events=events)
